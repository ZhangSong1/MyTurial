...
<body>
	...
	<div class="panel panel-primary">
		...
		<c:if test="${total%4 == 0}">
			<c:set value="${total/4 - 1}" var="cycles"></c:set>
		</c:if>
		<c:if test="${total%4 != 0}">
			<c:set value="${total/4}" var="cycles"></c:set>
		</c:if>
		<c:if test="${cycles>0}">
					...
					<c:forEach begin="0" end="${cycles}" varStatus="rowNum">
						<tr>
							<c:forEach items="${collectionsList}" var="item"
								...
								<td>
									<div class="row">
										<div style="background-image:url(${item.imgData})"
											class="col-sm-6 col-sm-offset-3 shortcutItem">
											<span class="shortcut-remove" style="display: none"
												data-id="${item.id}"><i class="icon-remove icon-2x"
												aria-hidden="true"></i></span> <span class="shortcut-edit"
												style="display: none"><i class="icon-edit icon-2x"
												aria-hidden="true"></i></span>
										</div>
									</div>
									<div class="row">
										<a href="javascript:void(0);" onclick="loadBody('${item.path}');" target="_blank"><span class="col-sm-10 col-sm-offset-1 text-center title">${item.name}</span></a>
									</div>
								</td>
								<c:if test="${tdNum.index == (total-1)}">
									<c:if test="${tdNum.index%4 < 3 && tdNum.index%4 != 0}">
										<c:forEach begin="${tdNum.index%4 + 1}" end="3">
											<td></td>
										</c:forEach>
									</c:if>
								</c:if>
